...
class TSNDataSet(data.Dataset):
    ...
    def _get_test_indices(self, record):
        ...
        if self.modality == 'Flow':
            ...
            interval = math.ceil(record.num_frames / self.new_length) # video length divded by 10;
            offsets = list()
            seg_range = np.linspace(0, record.num_frames, self.num_segments + 1, dtype='int64')
            offsets = seg_range[:-1]
    ...
    def __getitem__(self, index):
        ...
        if not self.test_mode:
            ...
            segment_indices = self._sample_indices_jhmdb(record) if self.random_shift else self._get_val_indices(record)
