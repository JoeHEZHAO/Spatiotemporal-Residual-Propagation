...
# from model_temp_res_gen import TSN
# from model_temp_res_gen_v2 import TSN
...
class TSN_BIT(nn.Module):
    ...
    def __init__(self):
        ...
        self.model_name = 'TSN_TemResGen_Kalman_2019-01-29_17-21-33.pth'
    ...
    def fea_gen_forward(self, input, batch_size, warmup_t, pred_t):
        x, y, _, _, _ = self.tsn.fea_gen_forward(input, batch_size, warmup_t, pred_t)
        return x, y
    ...
    def forward(self, input, warmup_t, pred_t):
        ...
        # gen_fea = torch.stack(gen_fea).transpose_(0, 1).contiguous().view(b_shape, 192, 28, 28)
        # org_fea = torch.stack(org_fea).transpose_(0, 1).contiguous().view(b_shape, 192, 28, 28)
...
for epoch in range(1):
    for idx, (input, target, indice) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            # gen_fea, org_fea = net.module.fea_gen_forward(input_var, b_shape, warmup_t, pred_t)
            # gen_fea = torch.stack(gen_fea).transpose_(0, 1).contiguous().view(b_shape * num_segments, -1, 28, 28)
            # org_fea = torch.stack(org_fea).transpose_(0, 1).contiguous().view(b_shape * num_segments, -1, 28, 28)

            '''
                use org_fea for testing if accuray is still the same;
                use gen_fea for testing our feature generating ability;
            '''
            # out = net.module.gen_fea_cls(gen_fea).detach()

            out = net(input_var, warmup_t, pred_t).detach()
